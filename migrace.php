...
try {
    ...
    if (MIGRACE)
        debug($S2_DB_CONFIG_temp, 'DB Config S2');
    if (MIGRACE)
        debug($S3_DB_CONFIG_temp, 'DB Config S3');
...
if (MIGRACE)
    write_ini_file(CLIENT_DIR . '/configs/klient.ini', $S3_nastaveni_klient);
if (MIGRACE)
...
if (TEST_KONTROLA == 1) {
    ...
    echo "\n\n";
    echo "  Celkový odhadovaný čas trvání migrace: <strong>" . number_format($cas, 3, ",", " ") . " sekund (" . number_format($cas / 60,
            0) . " minut a " . number_format($cas % 60, 0) . " sekund)</strong>.\n";
...
if (count($S2_spisove_znaky) > 0) {
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'spisovy_znak');
    foreach ($S2_spisove_znaky as $S2_sz) {
        ...
        if (MIGRACE):
            try {
                ...
                $S3->insert(S3_ . 'spisovy_znak',
                        array(
                    'id' => (int) $S2_sz->id_spisznak,
                    'parent_id' => null,
                    'nazev' => $nazev,
                    'popis' => (string) $S2_sz->popis,
                    'skartacni_znak' => !empty($S2_sz->skartacni_znak[0]) ? $S2_sz->skartacni_znak[0]
                                : null,
...
if (isset($_SESSION['S3_zpvyrizeni'])) {
    ...
    if (count($S2_vyriz) > 0) {
        if (MIGRACE)
            $S3->query('DELETE FROM ' . S3_ . 'zpusob_vyrizeni WHERE stav=2');
        foreach ($S2_vyriz as $S2_vy) {
            echo "\n>> " . htmlspecialchars($S2_vy->id_zpvyrizeni) . " = " . htmlspecialchars($S2_vy->nazev);
            if (MIGRACE):
                try {
                    $zpusob_vyrizeni = $S3->insert(S3_ . 'zpusob_vyrizeni',
                                    array(
                                'nazev' => $S2_vy->nazev,
                                'stav' => 0,
        ...
        if (MIGRACE)
            $_SESSION['S3_zpvyrizeni'] = $S3_zpvyrizeni;
    }
} // if session
unset($S2_vyriz);
...
if (count($S2_sub) > 0) {
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'subjekt');
        ...
        foreach ($S2_org as $S2_o) {
            ...
            if (MIGRACE):
                try {
                    ...
                    $plny_nazev = (empty($S2_o->nazev)) ? "Organizační jednotka č." . $S2_o->id_orgjednotka
                                : $S2_o->nazev;
                    $zkraceny_nazev = (empty($S2_o->nazev)) ? "Org. jednotka č." . $S2_o->id_orgjednotka
                                : $S2_o->nazev;
                    $sekvence = $zkraceny_nazev . "." . $S2_o->id_orgjednotka;

                    $org_id = null;
                    $org_id = $S3->insert(S3_ . 'orgjednotka',
                                    array(
...
if (count($S2_usr) > 0) {
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'user');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'osoba');
    if (MIGRACE)
            ...
            try {
                ...
                if ($osoba_id) {
                    ...
                    if ($user_id) {
                        ...
                        if ($S3->insert(S3_ . 'user_to_role',
                                        array(
                                    'role_id' => isset($role[(string) $S2_org][(string) $S2_role])
                                                ? $role[(string) $S2_org][(string) $S2_role] : 4,
                                    'user_id' => $user_id,
...
if (count($S2_dok) > 0) {
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'epodatelna');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'dokument');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'dokument_to_subjekt');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'dokument_to_spis');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'dokument_to_file');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'file');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'workflow');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'log_dokument');
    if (MIGRACE)
        $S3->query('TRUNCATE TABLE ' . S3_ . 'log_spis');
    ...
    foreach ($S2_dok as $S2_d) {
        ...
        if (strpos($S2_d->cislo_jednaci, '000EVIDENCE') !== false) {
            if (!empty($S2_d->id_isds) || !empty($S2_d->id_email)) {
                ...
                if (preg_match("/20(08|09|10|11|12)|^" . $S2_d->poradove_cislo . "/",
                                $S2_d->cislo_jednaci, $matches)) {
                ...
                $S2_odesilatele = $S2->query('SELECT do.*,o.nazev,o.ulice,o.mesto,o.psc,o.isds_idbox,o.cislo_popisne,o.cislo_orientacni,o.email FROM [:S2:dokumenty_a_odesilatele] AS do LEFT JOIN [:S2:odesilatele] AS o ON o.id_odesilatel=do.id_odesilatel WHERE do.id_dokument=%i',
                                $S2_d->id_dokument)->fetchAll();
                ...
                if (MIGRACE):
                    $S3->insert(S3_ . 'epodatelna',
                        ...
                        'identifikator' => null,
                        'predmet' => (empty($S2_d->strucny_obsah)) ? "Dokument " . $S2_d->cislo_jednaci
                                    : $S2_d->strucny_obsah,
                        'popis' => '' . $S2_d->poznamka,
        ...
        echo "\n<span style='color:blue;'>>> PRŮBĚH [zpracováno: " . $progress_aktual_per . "] v činnosti " . $progress_time_diff_f . " - odhadovaný čas konce: " . $progress_time_end_f . " (odhadem zbývá: " . number_format($progress_time_end - $progress_time_diff,
                2, '.', " ") . "s)</span>";
        echo "\n>> " . htmlspecialchars($S2_d->id_dokument . " = " . $S2_d->cislo_jednaci . " - " . $S2_d->strucny_obsah);

        // Podaci denik
        $podaci_rok = 0;
        if (preg_match("/20(08|09|10|11|12|13|14|15)|^" . $S2_d->poradove_cislo . "/",
                        $S2_d->cislo_jednaci, $matches)) {
            $podaci_rok = $matches[0];
        }
        ...
        try {
            ...
            if (MIGRACE)
                $S3->insert(S3_ . 'dokument', $dokument)->execute();
            if (MIGRACE)
                $dokument_id = (int) $S2_d->id_dokument;
            if (!MIGRACE)
                $dokument_id = 1;
            ...
            if ($dokument_id) {
                ...
                if (count($S2_odesilatele) > 0) {
                    foreach ($S2_odesilatele as $S2_do) {
                        if (MIGRACE):
                            $S3->insert(S3_ . 'dokument_to_subjekt',
                                    array(
                ...
                $S2_zaznamy = $S2->query('SELECT * FROM [:S2:zaznamy] WHERE id_dokument=%i ORDER BY datum',
                                $S2_d->id_dokument)->fetchAll();
                if (count($S2_zaznamy) > 0) {
                    foreach ($S2_zaznamy as &$S2_z) {
                        ...
                        if ($historie_stav[0] == "Dokument změněn")
                            continue;
                        ...
                        if (isset($historie_work[$historie_stav[0]])) {
                            ...
                            if (MIGRACE):
                                $workflow_id = $S3->insert(S3_ . 'workflow',
                                                array(
                                            'dokument_id' => $dokument_id,
                        ...
                        if (MIGRACE)
                            logDokument($S3, $dokument_id, @$historie[$historie_stav[0]],
                                    $S2_z->id_zamestnanec, $S2_z->datum,
                                    $S2_z->cinnost . ", " . $S2_z->poznamka);
                    }
                    if (!MIGRACE)
                        $workflow_id = 0;
                    if (isset($workflow_id) && $workflow_id > 0) {
                        $S3->update(S3_ . 'workflow',
                                array(
                            'stav_osoby' => 1,
                            'aktivni' => 1,
                ...
                if (!empty($S2_d->id_spis) && $S2_d->id_spis > 1) {
                    $S3_spis_count = $S3->query('SELECT poradi FROM [:S3:dokument_to_spis] WHERE spis_id=%i ORDER BY poradi DESC LIMIT 1;',
                                    $S2_d->id_spis + 1)->fetchSingle();
                    if (MIGRACE):
                        ...
                        $S3->insert(S3_ . 'dokument_to_spis',
                                array(
                            'dokument_id' => $dokument_id,
                ...
                $S2_prilohy = $S2->query('SELECT * FROM [:S2:prilohy] WHERE id_dokument=%i',
                                $S2_d->id_dokument)->fetchAll();
                if (count($S2_prilohy) > 0) {
                    foreach ($S2_prilohy as $S2_pr) {
                        ...
                        $real_path = str_replace("prilohy/", "/files/dokumenty/",
                                $S2_pr->umisteni);
                        ...
                        if ($priloha_id) {
                            ...
                            if (MIGRACE) :
                                if ($S3->insert(S3_ . 'dokument_to_file',
                                                array(
                                            'dokument_id' => $dokument_id,
                                            'file_id' => $priloha_id,
                                            'active' => 1,
                                            'date_added' => empty($S2_pr->vlozeno) ? new DateTime()
                                                        : $S2_pr->vlozeno,
...
if ($S3_cjednaci) {
    ...
    if (MIGRACE)
        $S3->insert(S3_ . 'cislo_jednaci', $S3_cjednaci)->execute();
