    ...
    public function InitCurl($logintype, $params)
    {
        ...
        if (!empty($params['proxy_host'])) {
            curl_setopt($this->ch, CURLOPT_PROXY,
                    'http://' . $params['proxy_host'] . ':' . $params['proxy_port']);
            curl_setopt($this->ch, CURLOPT_HTTPPROXYTUNNEL, true);
            if (!empty($params['proxy_login'])) {
                curl_setopt($this->ch, CURLOPT_PROXYUSERPWD,
                        $params['proxy_login'] . ':' . $params['proxy_password']);
            }
        }
...
class ISDSSoapClient extends SoapClient
{
    ...
    public function userAgent()
    {
        ...
        if (isset($curli['host']))
            $user_agent .= " (" . $curli['host'] . ")";
        if (isset($curli['version']))
            $user_agent .= " SoapClient via libcurl " . $curli['version'] . "";
        if (isset($curli['ssl_version']))
            $user_agent .= " (" . $curli['ssl_version'] . ")";
