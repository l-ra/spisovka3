...
class DERParser {
    ...
    public function parse($data)
    {
        ...
        //file_put_contents('asn/test.tmp', $data);
    ...
    private function _parse($data, $onelevel = 0, $start = 0)
    {
        ...
        while ( $i < strlen($data) ) {
            ...
            if ( $x2 == "83" ) {
                // 3bytes length
                $length_hex = sprintf("%02X%02X%02X",ord($data[$i+2]),ord($data[$i+3]),ord($data[$i+4]));
            ...
            if ( $tag->type == 1 ) {
                ...
                $intmp = $this->_parse(substr($data, $offset,$length),$onelevel,$offset+$start);
