...
class Auth_Component_SSO extends Authenticator_UI
{
    ...
    public function getAllUser()
    {
        ...
        if (!isset($ldap_params->ldap)) {
            throw new Nette\Security\AuthenticationException("Nedostupné nastavení nutné pro přihlášení.",
            self::FAILURE);
        }
    ...
    protected function ldap_parseEntries($info, $user = array())
    {
        ...
        for ($i = 0; $i < $info["count"]; $i++) {
            ...
            if (!empty($info[$i]['title'][0]))
                $funkce_a[] = $info[$i]['title'][0];
            if (!empty($info[$i]['department'][0]))
                $funkce_a[] = $info[$i]['department'][0];
            if (!empty($info[$i]['company'][0]))
                $funkce_a[] = $info[$i]['company'][0];
            ...
            if (!empty($info[$i]['telephonenumber'][0]))
                $tel_a[] = $info[$i]['telephonenumber'][0];
            if (!empty($info[$i]['mobile'][0]))
                $tel_a[] = $info[$i]['mobile'][0];
    ...
    public function render()
    {
        ...
        if ($this->action != "login")
            ...
            if (Nette\Environment::getHttpRequest()->getCookie('s3_logout')) {
                ...
                header("Location: " . Nette\Environment::getVariable('baseUri') . "auth/logout.php",
                        302);
                exit;
            }
            ...
            if (@$_SESSION['s3_auth_remoteuser']) {
                ...
                try {
                    ...
                    header("Location: " . Nette\Environment::getVariable('klientUri',
                                    Nette\Environment::getVariable('baseUri')), 302);
                ...
                $base_url = Nette\Environment::getVariable('klientUri',
                                Nette\Environment::getVariable('baseUri'));
        ...
        if (is_array($seznam)) {
            ...
            foreach ($seznam as $id => $user) {
                if (!isset($user_seznam[$user['uid']])) {
                    ...
                    echo "    </td>\n";
                    echo "    <td>\n";
                    echo "       " . $this->mySelect("usersynch_role[" . $id . "]",
                            $role_seznam, 2) . "\n";
                    echo "    </td>\n";
                    echo "    <td>\n";
    ...
    public function handleUserRegistration($data)
    {
        ...
        if (!isset($_SESSION['s3_auth_remoteuser'])) {
            $this->presenter->flashMessage('Nepodařilo se získat uživatelské jméno přihlašeného uživatele!',
                    'warning');
